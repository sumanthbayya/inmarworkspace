 ...
 * @private
 * @param {Object}
 *            global - The root context (window/global/...)
 * @param {function}
 *            factory - Returns a new instance of the API
...
(function initWebNotification(global, factory) {
	 ...
	 * @private
	 * @param {Object}
	 *            context - The root context (window/global/...)
	 * @returns {Object} New instance of the API
	 */
	 ...
	 * @public
	 */
    webNotification.allowRequest = true; // true to enable automatic
											// requesting of permissions if
											// needed
    ...
    /**
	 * Displays the web notification and returning a 'hide' notification
	 * function.
	 * 
	 * @function
	 * @memberof! webNotification
	 ...
	 * @private
	 * @param {String}
	 *            title - The notification title text (defaulted to empty string
	 *            if null is provided)
	 * @param {Object}
	 *            options - Holds the notifhideication data (web notification API
	 *            spec for more info)
	 * @param {String}
	 *            [options.icon=/favicon.ico] - The notification icon (defaults
	 *            to the website favicon.ico)
	 * @param {Number}
	 *            [options.autoClose] - Auto closes the notification after the
	 *            provided amount of millies (0 or undefined for no auto close)
	 * @param {function}
	 *            [options.onClick] - An optional onclick event handler
	 * @param {Object}
	 *            [options.serviceWorkerRegistration] - Optional service worker
	 *            registeration used to show the notification
	 * @param {ShowNotificationCallback}
	 *            callback - Invoked with either an error or the hide
    ...
    var createAndDisplayNotification = function (title, options, callback) {
        ...
        if (!options.icon) {
            options.icon = 'C:/Users/kotavasudeva/EsipUiLatest/EsipUiLatest/src/main/resources/static/assets/images/logout.ico';
        }
	 ...
	 * @private
	 * @param {Array}
	 *            argumentsArray - An array of all arguments provided to the
	 ...
	 * The callback invoked will get an error object (in case of an error, null
	 * in case of no errors) and a 'hide' function which can be used to hide the
	 * notification.
	 * 
	 * @function
	 * @memberof! webNotification
	 ...
	 * @public
	 * @param {String}
	 *            [title] - The notification title text (defaulted to empty
	 *            string if null is provided)
	 * @param {Object}
	 *            [options] - Holds the notification data (web notification API
	 *            spec for more info)
	 * @param {String}
	 *            [options.icon=/favicon.ico] - The notification icon (defaults
	 *            to the website favicon.ico)
	 ...
	 *            registeration used to show the notification
	 * @param {ShowNotificationCallback}
	 *            [callback] - Called after the show is handled.
	 * @example ```js //show web notification when button is clicked
	 *          $('.some-button').on('click', function onClick() {
	 *          webNotification.showNotification('Example Notification', { body:
	 *          'Notification Text...', icon: 'my-icon.ico', onClick: function
	 *          onNotificationClicked() { console.log('Notification clicked.'); },
	 *          autoClose: 4000 //auto close the notification after 4 seconds
	 *          (you can manually close it via hide function) }, function
	 *          onShow(error, hide) { if (error) { window.alert('Unable to show
	 *          notification: ' + error.message); } else {
	 *          console.log('Notification Shown.');
	 * 
	 * setTimeout(function hideNotification() { console.log('Hiding
	 * notification....'); hide(); //manually close the notification (you can
	 * skip this if you use the autoClose option) }, 5000); } }); });
	 * 
	 * //service worker example
	 * navigator.serviceWorker.register('service-worker.js').then(function(registration) {
	 * $('.some-button').on('click', function onClick() {
	 * webNotification.showNotification('Example Notification', {
	 * serviceWorkerRegistration: registration, body: 'Notification Text...',
	 * icon: 'my-icon.ico', actions: [ { action: 'Start', title: 'Start' }, {
	 * action: 'Stop', title: 'Stop' } ], autoClose: 4000 //auto close the
	 * notification after 4 seconds (you can manually close it via hide
	 * function) }, function onShow(error, hide) { if (error) {
	 * window.alert('Unable to show notification: ' + error.message); } else {
	 * console.log('Notification Shown.');
	 * 
	 * setTimeout(function hideNotification() { console.log('Hiding
	 * notification....'); hide(); //manually close the notification (you can
	 * skip this if you use the autoClose option) }, 5000); } }); }); }); ```
	 */
    webNotification.hide=function(notification){
    	notification.close();
    	/*getNotifications({
            tag: tag
        }).then(function notificationsFetched(notifications) {
